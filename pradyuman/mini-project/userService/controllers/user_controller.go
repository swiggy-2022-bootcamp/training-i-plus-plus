...
func GetAUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, responses.UserResponse{Status: http.StatusBadRequest, Message: "Invalid User Id"})
			return
		}
...
func DeleteAUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, responses.UserResponse{Status: http.StatusInternalServerError, Message: err.Error()})
			return
		}

		if result.DeletedCount < 1 {
			c.JSON(http.StatusNotFound,
				responses.UserResponse{Status: http.StatusNotFound, Message: "User with specified ID not found!"},
			)
			return
		}

		c.JSON(http.StatusOK,
			responses.UserResponse{Status: http.StatusOK, Message: "User successfully deleted!"},
		)
	}
}
...
func IsUserAuthorized() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token.Claims.(jwt.MapClaims)["role"] != "BUYER" {
