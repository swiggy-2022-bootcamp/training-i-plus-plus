...
//	fmt.Println("\nInside complete payment!")
	    // cartId := primitive.ObjectIDFromHex(c.Param("cart_id"))
		 var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		 defer cancel()
		 //var trackstreamobj= model.TrackStream
		 count1, err := trackstreamCollection.CountDocuments(ctx,  bson.M{"payment_type":"DIGITAL"})
		 count2, err := trackstreamCollection.CountDocuments(ctx,  bson.M{"payment_type":"COD"})
		 if err != nil {
